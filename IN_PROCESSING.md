...
Parent Github repo is at: https://github.com/prusa3d/PrusaSlicer
...
Analyze the parent repository's open and closed issues and PRs, recent commit history, and any roadmap or discussion threads. Even if there are thousands, go through all of them. Apply critical thinking to sort: which requests are still relevant, which issues are duplicates or already fixed, which features fit the project's trajectory, which PRs are worth merging. Save your progress as you go.
